...
// The event director needs both the first and last names of each runner for their 
// running bibs. Combine both the first and last names and populate a new array called 
// `fullNames`. This array will contain just strings.
let fullNames = [];
runners.forEach((runners) => {
  fullNames = runners.first_name + runners.last_name;
...
// because the director BECAME DRUNK WITH POWER. Populate an array called 
// `firstNamesAllCaps`. This array will contain just strings.
let firstNamesAllCaps = [];
runners.map((runners) => {
  firstNamesAllCaps = runners.first_name;
...
// with large sized shirts so they can choose a different size. This will be an array 
// of objects.
let runnersLargeSizeShirt = runners.filter((shirt) => {
...
// donations and save the total into a ticketPriceTotal variable.
